...
public class game_logic implements Consta {
  ...
  /*
   * public void saveReplay(long mod) { Vector ms,time;
   * 
   * RandomAccessFile file = null; try { if(KolT==1){ file = new
   * RandomAccessFile("F:\\morskoy\\save.txt", "rw"); } else{ file = new
   * RandomAccessFile("F:\\morskoy\\save.txt", "a"); } } catch (Throwable e) { } try { if (mod == 1)
   * { for (int i = 0; i < 10; i++) { for (int j = 0; j < 10; j++) { file.writeInt(masP[i][j]);
   * file.writeInt(masC[i][j]); file.writeLong(mod); } } }
   * 
   * 
