...
//Game panel(Buttons,Label,playing field)
public class game_panel extends JPanel implements Consta {
  ...
  /*
   * private void loadReplay(){ int i, j; long t; RandomAccessFile file = null; try { file = new
   * RandomAccessFile("F:\\morskoy\\data.txt", "r"); } catch (Throwable e) { } try { for (i = 0; i <
   * 10; i++) { for (j = 0; j < 10; j++) { myGame.masP[i][j] = file.readInt(); myGame.masC[i][j] =
   * file.readInt(); } } t=file.readLong(); myGame.Pause(t); repaint(); file.close(); } catch
   * (IOException e) { } }
   */
}
